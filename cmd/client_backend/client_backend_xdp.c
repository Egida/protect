                    ...
                    if ( ip->daddr == config->public_address && udp->dest == config->port )
                    {
                        ...
                        switch ( packet_data[0] )
                        {
                            ...
                            case NEXT_CLIENT_BACKEND_PACKET_REFRESH_TOKEN_REQUEST:
                            {
                                ...
                                result = proton_secretbox_encrypt( (__u8*) &response->backend_token, sizeof(struct next_client_backend_token_t), 0, client->client_backend_private_key, PROTON_SECRETBOX_KEY_BYTES );
                                if ( result != 0 )
                                {
                                    debug_printf( "could not encrypt backend token" );
                                    return XDP_DROP;
